...
assign.neighbors <- function(NN, rand=FALSE, sample.indx=which(s>0)){
    ...
    ## print("used not in sample")
    ## print(used.neighbors[!(used.neighbors %in% sample.indx)])
    ...
    assignment.vec[positions.filled] <- used.neighbors[!(used.neighbors %in% sample.indx)]
...
run.test.cases <- function(){
    ...
    test.2.result <- all.equal(c(3, 2, 0), assign.neighbors(NN=list(nn.index=test.case.2,
